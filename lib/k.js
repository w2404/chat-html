...
//记录发送的message总长度
let message_length
//记录服务器计算出的tokens和消息长度的比例
let message_length_over_tokens
    ...
    methods: {
        ...
        */
        ...
        send() {
            ...
            if (  this.messages.reduce((acc,o) => acc+o.content.length,0)  > this.tokens_threshold * message_length_over_tokens){
                this.savemessages()
                this.error += '因为tokens长度超出'+this.tokens_threshold+',字符串长度超过'+this.tokens_threshold * message_length_over_tokens+'而保存文件。<br/>'
            }
            while(  this.messages.reduce((acc,o) => acc+o.content.length,0)  > this.tokens_threshold * message_length_over_tokens){
                this.messages.shift()
                this.error += '删除一条超出tokens长度限制的顶部对话。剩余字符串长度'+(this.messages.reduce((acc,o) => acc+o.content.length,0))+'<br/>'
            }
            //记录发送的message总长度
            message_length=this.messages.reduce((acc,o) => acc+o.content.length,0)
            ...
            this.sending = $.post({
                ...
                success: data => {
                    ...
                    //这里我们假设只有一个choice
                    message_length_over_tokens=((message_length+data.choices[0].message.content.length)/data.usage.total_tokens)
